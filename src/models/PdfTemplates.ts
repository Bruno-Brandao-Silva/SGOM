...
const purchasePDF = ({ purchase, purchaseList, info, client, addresses, contacts }: {
	purchase: Purchase,
	...
	const totalPrice = purchaseList.reduce((acc, item) => {
		return acc + (item.price * item.quantity)
	}, 0)
		...
		content: [
			...
			},
			{
				table: {
					...
					body: [
						[
							{ text: [{ text: `Venda:`, style: "defText" }, `  Nº${purchase.id}`] },
							{
								text: [{ text: `Data:`, style: "defText" }, ` ${purchase.date.replace(/(\d{4})-(\d{2})-(\d{2})/, "$3/$2/$1")}`],
								alignment: 'center',
								margin: [10, 0, 10, 0]
			...
			require_list_table,
			{
				...
				table: {
					...
					body: [
						[
							{ text: `Garantia do serviço: ${1} meses. Peças conforme fabricante.` },
							{
								text: [
									{ text: `Total:`, style: "defText" },
									` ${Utils.monetaryMask(totalPrice)}`
...
const pdfTemplate = { servicePDF, purchasePDF }
