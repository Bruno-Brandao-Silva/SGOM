import { app, BrowserWindow, dialog, MessageBoxReturnValue, ipcMain } from 'electron';
import fs from 'fs';
...
const createWindow = (): void => {
	...
	ipcMain.handle("pdfCreator", async (event, arg: { docDefinition: TDocumentDefinitions, docName: string, dir: string, options?: BufferOptions }) => {
		try {
			...
			pdfDoc.end();
			return dir + arg.docName + '.pdf'
		} catch (error) { console.log(error); return false }
	});

	ipcMain.handle('printer', async (event: any, arg: { path: string }) => {
		try {
			...
			await print(arg.path, { printer: printers[response].deviceId })
			return "success"
		} catch (error) {
			return error
		}
